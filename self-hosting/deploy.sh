...
# --------------------------------------------------------- warmup UUIDs
# AIOMetadata's comprehensive catalog warmer needs the config UUIDs it should
# warm, and those are generated by the app on first save - so they can't live
# in the repo. Read them straight out of its database instead: nothing to
# remember, and a rebuilt server picks up its own UUIDs on the next deploy.
c "Cache warmup"
AIOMETA_DB="$STACK_DIR/data/aiometadata/data/db.sqlite"
CACHE_WARMUP_UUIDS=""
# The LIKE filter keeps only real config UUIDs. AIOMetadata seeds its own
# internal 'system-cache-warmer' row, which has no catalogs of its own and
# would otherwise occupy one of the three warmup slots.
uuid_where="user_uuid LIKE '%-%-%-%-%'"
uuid_count=$(ssh "$TARGET" \
  "sqlite3 -readonly '$AIOMETA_DB' \"SELECT COUNT(*) FROM user_configs WHERE $uuid_where;\" 2>/dev/null" \
  || true)
uuid_count=${uuid_count:-0}

if [[ $uuid_count =~ ^[0-9]+$ ]] && (( uuid_count > 0 )); then
  # AIOMetadata accepts at most 3 UUIDs; take the oldest configs.
  CACHE_WARMUP_UUIDS=$(ssh "$TARGET" \
    "sqlite3 -readonly '$AIOMETA_DB' \
     \"SELECT group_concat(user_uuid) FROM (SELECT user_uuid FROM user_configs WHERE $uuid_where ORDER BY id LIMIT 3);\" 2>/dev/null" \
    || true)
  if (( uuid_count > 3 )); then
    inf "$uuid_count configs found; warming the 3 oldest (addon limit)"
  else
    ok "warming $uuid_count config(s)"
  fi
else
  inf "no configs in the addon database yet - warming stays off until there is one"
fi
...
{
  grep -vE '^(SECRET_KEY|AIOSTREAMS_AUTH|AIOSTREAMS_BASICAUTH|AIOMETADATA_AUTH|STACK_USER|CF_DNS_API_TOKEN|CACHE_WARMUP_UUIDS)=' "$LOCAL_ENV"
  echo
  ...
  echo "CACHE_WARMUP_UUIDS=$CACHE_WARMUP_UUIDS"
